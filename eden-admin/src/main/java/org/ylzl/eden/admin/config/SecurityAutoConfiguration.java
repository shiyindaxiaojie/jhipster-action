...
import org.ylzl.eden.admin.filter.OAuth2ClientCredentialsFilter;
...
public class SecurityAutoConfiguration {
	...
	public static class JwtWebSecurityAutoConfiguration extends JwtWebSecurityConfigurerAdapter {
		...
		public JwtWebSecurityAutoConfiguration(JwtTokenProvider jwtTokenProvider, JwtProperties jwtProperties,
											   WebEndpointProperties webEndpointProperties, AdminServerProperties adminServerProperties,
...
//			super.configure(http);
			...
			http.authorizeRequests((authorizeRequests) -> authorizeRequests
					...
					// Zuul
					.antMatchers(this.zuulProperties.getPrefix() + PathMatcherConstants.ALL_CHILD_PATTERN).authenticated()
					...
					// Spring Boot Admin
					.antMatchers(this.adminServerContextPath + "/assets/**").permitAll()
					.antMatchers(this.adminServerContextPath + "/login").permitAll()
					.anyRequest().authenticated())
				    .formLogin((formLogin) -> formLogin.loginPage(this.adminServerContextPath + "/login").successHandler(successHandler))
				    .logout((logout) -> logout.logoutUrl(this.adminServerContextPath + "/logout"))
				    .csrf((csrf) -> csrf.csrfTokenRepository(CookieCsrfTokenRepository.withHttpOnlyFalse())
					.ignoringRequestMatchers(
						new AntPathRequestMatcher(this.adminServerContextPath + "/instances", HttpMethod.POST.toString()),
						new AntPathRequestMatcher(this.adminServerContextPath + "/instances/**", HttpMethod.DELETE.toString()),
	...
	public static class OAuth2WebSecurityAutoConfiguration {
		...
		public OAuth2ClientCredentialsFilter oAuth2ClientCredentialsZuulFilter(ClientCredentialsTokenHolder clientCredentialsTokenHolder,
			...
			return new OAuth2ClientCredentialsFilter(clientCredentialsTokenHolder,adminServerProperties, oAuth2Properties, pathMatcher);
		}
	}
}
